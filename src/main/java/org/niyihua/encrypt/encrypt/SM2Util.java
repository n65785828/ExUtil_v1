...
public class SM2Util extends GMBaseUtil implements Encrypt{
    //////////////////////////////////////////////////////////////////////////////////////
    /*
     * 以下为SM2推荐曲线参数
     */
    ...
    public static void main(String[] args) {
        AsymmetricCipherKeyPair asymmetricCipherKeyPair = generateKeyPairParameter();
        ECPublicKeyParameters aPublic = (ECPublicKeyParameters) asymmetricCipherKeyPair.getPublic();
        ECPrivateKeyParameters aPrivate = (ECPrivateKeyParameters) asymmetricCipherKeyPair.getPrivate();
        System.out.println("公钥");
        System.out.println(ByteUtils.toHexString(((aPublic.getQ().getEncoded(false)))));
        System.out.println("私钥");
        System.out.println(ByteUtils.toHexString(((aPrivate.getD().toByteArray()))));
    }
    ...
    public static String encryptString(String raw, String publicKey) {
        ...
        try {
            return DatatypeConverter.printHexBinary(encrypt(k, raw.getBytes(StandardCharsets.UTF_8)));
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static String decryptString(String cipher, String privateKey) {
        ...
        try {
            return new String(decrypt(k, DatatypeConverter.parseHexBinary(cipher)), StandardCharsets.UTF_8);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public String encrypt(String plain, String publicKey) {
        return encryptString(plain,publicKey);
    }

    @Override
    public String decrypt(String cipher, String privateKey) {
        return decryptString(cipher,privateKey);
    }
}
